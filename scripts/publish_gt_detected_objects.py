...
class DetectedObjectsBboxPublisher:
    ...
    def __init__(self, configs, output_topic: str):
        """
        :param vehicle_name: name of the vehicle model
        :param object_name: name of the object model
        :param camera_info: camera info message
        :param output_topic: topic to publish the detected objects to
        :param object_name: name of the object
        :param object_dimensions: dimensions of the object
        :param camera_pose: camera pose in the world frame (x, y, z, qx, qy, qz, qw)
        """
        self.configs = configs
        self.output_topic = output_topic
        self.pub = rospy.Publisher(
            self.output_topic, DetectedObjects, queue_size=1)
    ...
    def get_detected_object(self, config: Config):
        ...
        camera_transform: tf2_ros.TransformStamped = self.tf_buffer.lookup_transform(config.camera_link, "world", rospy.Time(), rospy.Duration(1.0))
        ...
        img_points = cv2.projectPoints(objectPoints, cv2.Rodrigues(camera_R[:3, :3])[0], camera_trans,
                                       np.array(config.camera_info.K).reshape(3, 3), np.array(config.camera_info.D), None)[0].squeeze()
...
if __name__ == "__main__":
    ...
    front_camera_info = rospy.wait_for_message(
        "/auv4/front_cam/camera_info", CameraInfo, timeout=2)
    ...
    configs = [
        ...
        Config("auv4/front_cam_optical", "robosub_torpedo_bootlegger", front_camera_info, "Bootlegger", object_centre = [0, 1, 1.11], object_dimensions=[0.4, 0, 0.8])
    ]
    motion = DetectedObjectsBboxPublisher(configs, "detected_objects")
    # motion = DetectedObjectsBboxPublisher("auv4/front_cam_optical", "robosub_torpedo_bootlegger",
    #                                       camera_info, "detected_objects", "Bootlegger", object_centre = [0, 0, 1.1], object_dimensions=[0.7, 0, 1.6])
    rospy.spin()
